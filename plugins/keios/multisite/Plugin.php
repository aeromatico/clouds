...
class Plugin extends PluginBase
{
    ...
    public function boot()
    {
        ...
        foreach ($binds as $domain => $bind) {
            if (preg_match('/'.preg_quote($backendUri, '/').'/', $requestUrl) && preg_match(
                    '/'.preg_quote($currentHostUrl, '/').'/i',
                    $domain
                ) && $bind['is_protected']
            ) {
         ...
         */
        if (preg_match('/'.preg_quote($backendUri, '/').'/', $requestUrl)) {
            return null;
        }
        /*
        ...
        Event::listen(
            ...
            function () use ($binds, $currentHostUrl) {
                foreach ($binds as $domain => $bind) {
                    if (preg_match('/'.preg_quote($currentHostUrl, '/').'/i', $domain)) {
